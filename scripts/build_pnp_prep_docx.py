...
def build() -> Path:
    ...
    add_body(doc, "Распечатайте четыре файла (ссылки подставите сами):")
    add_list(
        doc,
        [
            "Правила: *** Формат A4, книжная ориентация. Цвет не обязателен. Обычная офисная бумага. Не режьте этот файл.",
            "Карты запросов (лицо) и «Ты восхитителен»: *** Формат A4, альбомная ориентация. Цветная печать крайне желательна (цвета игроков на картах «Ты восхитителен»). Карты – на плотной бумаге.",
            "Карты запросов (оборот): *** Формат A4, альбомная ориентация. Тот же лист, что и лица запросов. Карты – на плотной бумаге.",
    ...
    add_body(doc, "Только карты запросов двусторонние. Лица и обороты лежат в разных файлах.")
    add_body(doc, "В файле лиц:")
    add_list(
        doc,
        [
            "страница 1 – «Карты запросов · ЛИЦО»;",
            "страница 2 – «Карты «Ты восхитителен»», только лицо, без оборота.",
        ],
        numbered=False,
    )
    add_body(
        doc,
        "В файле оборотов одна страница – «Карты запросов · ОБОРОТ». Напечатайте страницу 1 файла лиц и страницу файла оборотов на двух сторонах одного листа. Переворот выберите так, чтобы верх листа остался верхом: после печати текст на обороте читается без поворота страницы. Если лица и рубашки не совпали – смените сторону переворота в настройках принтера.",
    )
    add_body(
        doc,
